<template>
  <div class="relative min-h-screen">
    <ParallaxHeader :imageUrl="accueilImage" imageAlt="vue aérienne de la cour de l'ENM" height="50vh" />
    ...
    <section class="content bg-blanc -mt-5 py-8 px-5 z-10 relative border border-blanc rounded-3xl h-[45vh] shadow-lg inset-shadow-sm">
      ...
      <h1 class="w-18/20">Plongez dans l’histoire de l’ENM</h1>
      <p>L’École nationale de la magistrature occupe un site historique qui présente la double particularité d’être un
        lieu de justice depuis plus de cinq siècles en même temps qu’un monument clé de l’histoire de Bordeaux.</p>
    ...
    <section class="content bg-marron -mt-4 pb-15 pt-12 py-8 px-5 z-5 relative border border-blanc">
      ...
      <div class="relative w-full max-w-4xl mx-auto mb-6 mt-6">
        ...
        <component v-for="pin in pins" :key="pin.id" :is="pin.active ? IconQrActif : IconQrInactif"
          class="absolute cursor-pointer shadow-lg inset-shadow-sm rounded-2xl" :style="{
            top: pin.y + 'px',
            left: pin.x + 'px',
      ...
      <Transition name="fade-card" mode="out-in">
        <div v-if="activePin">
          <CardHome :title="activePin.title" :etape="activePin.etape" :img="activePin.img" :alt="activePin.alt"
            :link="activePin.link" />
        </div>
      </Transition>
    ...
    <section class="content -mt-5 bg-blanc py-8 px-5 z-10 relative border border-blanc rounded-3xl">
      ...
      <p>Le parcours est découpé en trois catégories qui abordent chacune des points particuliers de l’École nationale
        de la magistrature.</p>
      <BulletFleche class="mt-4">
        <h3 class="mt-0">Une école pour une Justice moderne</h3>
      </BulletFleche>
      <p>Découvrez comment l’École nationale de la magistrature a vu le jour, à travers quelques dates clés et
        événements majeurs. Cette partie vous permet de comprendre les origines et le rôle de cette institution.</p>

      <BulletFleche class="mt-4">
        <h3 class="mt-0">Architecture et patrimoine</h3>
      </BulletFleche>
      <p>Partez à la découverte des bâtiments de l’École et de leur évolution dans le temps. Certains éléments
        architecturaux remarquables, comme la Tour des Minimes ou la Tour des Sorcières, vous seront présentés,
        accompagnés d’anecdotes sur la conception du site.</p>

      <BulletFleche class="mt-4">
        <h3 class="mt-0">Une formation exigeante et humaine</h3>
      </BulletFleche>
      <p>Plongez dans le quotidien de l’École à travers les récits de celles et ceux qui l’ont vécue&nbsp;: élèves
        magistrats, et visiteurs… Ces témoignages offrent un aperçu vivant et concret de l’expérience au sein de
        l’institution.</p>

      <Bouton label="Commencer l'exploration" to="/scan" />
    </section>
  </div>
</template>
